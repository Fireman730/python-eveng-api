...
# In progress ...
# Check that ethernet: number is higher that port connection number
...
def validateYamlFileForPyEVENG(yamlContent: dict(), apiEVENG):
    ...
    assert checkDeviceElement(CONSOLE_TYPES, yamlContent, paramToVerify="console")
    # Check that configs type is in CONFIG_TYPES
    assert checkDeviceElement(CONFIG_TYPES, yamlContent, dictToVerify="configs", paramToVerify="console")
...
def checkDeviceElementWithPath(listElement: list(), pathToYamlFile: str(), dictToVerify: str()="devices", paramToVerify: str() = "type") -> bool:
    return checkIfImageIsAvaiable(listElement, open_yaml_files(pathToYamlFile), dictToVerify, paramToVerify)
