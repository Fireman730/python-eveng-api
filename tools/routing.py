...
IPTABLES_DNAT = "iptables -A PREROUTING -t nat -i $(ip route show | grep $(hostname -I | awk '{{print $1}}') | awk '{{print $3}}') -p tcp --dport {} -j DNAT --to {}:{}"
IPTABLES_ALLOWED = "iptables -A FORWARD -p tcp -d $(hostname -I | awk '{{print $1}}') --dport {} -j ACCEPT"
IPTABLES_SNAT = "iptables -A POSTROUTING -t nat -o {} -p tcp --dport 22 -j SNAT --to {}"
...
# ifconfig | grep encap | awk '{print $1}'
